...
For videos, thumbnails and 5-second previews are generated via AWS Batch (Fargate Spot) using FFmpeg. See ADR-012 for the video processing pipeline decision.
...
- **Amendment (2026-03-16)**: The 800×800 JPEG rule applies to standard raster formats. Animated GIFs are resized but output as GIF (Sharp's rotate/toColorspace pipeline is skipped to preserve frames and the palette). WebP files (including animated) are output as resized WebP at quality 80 (`animated: true`). This ensures animated thumbnails remain animated in the photo grid.
- **Amendment (2026-03-14)**: Thumbnails are now served via CloudFront signed URLs (ADR-013) rather than S3 presigned URLs. The thumbnail stays in S3 Standard; only the delivery mechanism changed.
- Videos do not have thumbnails generated by Sharp — `previewKey` column added (migration 0013) for the FFmpeg-generated preview. The frontend renders a `<video>` element with `previewUrl` on hover.
