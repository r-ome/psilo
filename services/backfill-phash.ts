...
async function main() {
  ...
  while (true) {
    const batch = await db
      ...
      .from(photos)
      .where(and(isNull(photos.phash), isNotNull(photos.thumbnailKey), notLike(photos.contentType, "video/%")))
      .limit(BATCH_SIZE)
      .offset(offset);
