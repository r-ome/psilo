...
async function handlePostAlbums(
...
async function handlePostAlbumPhoto(
  event: APIGatewayProxyEventV2WithJWTAuthorizer,
  db: ReturnType<typeof createDb>,
  sub: string,
): Promise<APIGatewayProxyResultV2> {
...
export const handler = async (
  ...
  switch (routeKey) {
    case 'POST /albums':
      return handlePostAlbums(event, db, sub);
    case 'GET /albums':
      return handleGetAlbums(db, sub);
    case 'GET /albums/{albumId}':
      return handleGetAlbum(event, db, sub);
    case 'POST /albums/{albumId}/photos':
      return handlePostAlbumPhoto(event, db, sub);
    case 'DELETE /albums/{albumId}/photos/{photoId}':
      return handleDeleteAlbumPhoto(event, db, sub);
    case 'DELETE /albums/{albumId}':
      return handleDeleteAlbum(event, db, sub);
    case 'PUT /albums/{albumId}':
      return handlePutAlbum(event, db, sub);
    default:
